...
`/mabi-start` 를 치면 연결 상태·레벨·가방·정령의 날개를 한 번에 보여 주고 메뉴 3개를 고르게 한다. 전부 무료 조회다.
...
### 2) 생활 레벨 — 날개 대비 경험치가 좋은 방법을 제안

- "낚시 레벨 올리고 싶어"처럼 말하면 지금 캘 수 있는 것과 공짜 경험치(주말 아르바이트·납품 퀘스트)를 먼저 짚어 준다.
- 커넥터는 생활 스킬 레벨을 알려 주지 않으므로 현재 레벨은 사용자가 말해 준다.

### 3) 채집·제작 — 여기서만 정령의 날개가 든다
...
| 질문 | 답 |
|---|---|
| 상태·숙제 확인에도 날개가 드나? | 아니다. 조회는 전부 무료. 날개는 채집·제작·가공을 **맡길 때만** 5개. |
...
| 가방이 꽉 찼다고 하면? | 가방 정리는 직접. 95% 이상이면 채집을 시작하지 않는다(날개는 안 나간다). |
