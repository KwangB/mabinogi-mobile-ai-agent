...
2. 받기:
   ...
   ```
3. 점검(게임 없이): 폴더 안에서 `node server/test/smoke.mjs` → 마지막 줄 `36/36 passed`.
...
### 2) 생활 레벨 — 최적 목표를 스스로 골라 실행 (실행 1회 = 날개 5개)
...
### 3) 채집·제작 — 원하는 것을 콕 집어서 (실행 1회 = 날개 5개)
...
- "타르 만들게 재료 채워 줘"처럼 말하면 부족 재료를 읽어 채집 호출 수와 비용을 표로 보여 주고, 승인 뒤 채집 → 가공/제작까지 순서대로 진행한다. 가공 등록은 경험치가 없어 게임에서 직접 거는 걸 권한다.
...
| [server/](server/) | 공식 CLI 를 그대로 호출하는 MCP 서버(Node 18+, 의존성 없음) + 모의 CLI + 테스트 36종 |
...
```
게임 없이 36개 항목을 검사한다(마지막 줄 `36/36 passed`).
